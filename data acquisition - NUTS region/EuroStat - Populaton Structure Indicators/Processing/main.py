...
input_file = "data.csv"
output_file = "../structure_indicators.csv"

cities_file = "../../EuroStat (Meta) - Cities/city_codes.csv"

cities_data = pd.read_csv(cities_file, sep=";", header=0,
                            index_col=0, encoding="utf-8")
input_file = pd.read_csv(input_file, sep=",", header=0,
                            index_col=0, encoding="utf-8")
merged_data = pd.merge(input_file, cities_data, left_on="geo", right_on="code")

merged_data = merged_data[["city", "geo", "indic_de", "unit", "TIME_PERIOD", "OBS_VALUE"]]
merged_data = merged_data.rename(columns={"city": "city_name", "geo": "city_code", "indic_de": "Indicator", "unit": "unit", "TIME_PERIOD": "year", "OBS_VALUE": "value"})

merged_data["Indicator"] = merged_data["Indicator"].map(di_dict)
merged_data["unit"] = merged_data["unit"].map(unit_dict)

merged_data.to_csv(output_file, sep=";", header=True, index=False, encoding="utf-8")
